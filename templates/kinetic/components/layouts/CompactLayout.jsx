...
export default function CompactLayout({ items, dataType, itemType }) {
  ...
  useGSAP(
    () => {
      ...
      const batch = revealItems(root, '.kinetic-compact-slot', {
        y: 22,
        x: -6,
        stagger: 0.05,
        scale: 0.985,
  ...
  return (
    <div
      className={`kinetic-compact${isMinimal ? ' is-minimal' : ''}`}
      ref={rootRef}
      data-kinetic-items
    >
      {items.map((item, index) => {
        ...
        return (
          <KineticMotionItem
            ...
            className="kinetic-compact-slot"
            lift={-7}
            scale={1.02}
          >
            <span className="kinetic-slot-index">{String(index + 1).padStart(2, '0')}</span>
            <div className="kinetic-slot-body">
            ...
            {!isMinimal && fields.links.length > 0 && (
              <div className="kinetic-slot-links">
