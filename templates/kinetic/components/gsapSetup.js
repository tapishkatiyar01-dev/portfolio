...
export function registerKineticGsap() {
...
export function revealSection(plane, options = {}) {
  ...
  const parts = Array.from(plane.querySelectorAll('[data-reveal]')).filter(
    (node) => !node.closest('[data-kinetic-items]'),
  );
 ...
 * and reverses when scrolled past upward — smooth replay, pagination-safe.
 */
export function revealItems(container, childSelector, options = {}) {
  ...
  const x = options.x ?? 0;
  const scale = options.scale ?? 0.975;
  const rotate = options.rotate ?? 0;
  const rotateX = options.rotateX ?? 5;
  ...
  return {
    ...
    kill() {
      ...
      g.set(children, { clearProps: 'transform,opacity' });
    },
  };
}
