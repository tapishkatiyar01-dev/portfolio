...
/**
 * For scroll-stack themes: restore last section on load + write cookie while scrolling.
 */
export function usePersistedSectionScroll(template, sectionIds, {
  ...
  const [sectionId, setSectionId] = usePersistedSectionId(template, sectionIds, {
    initialValue,
    fallback: fallback || sectionIds[0] || 'about',
  });
  const restoredRef = useRef(false);

  useEffect(() => {
    if (restoredRef.current || typeof window === 'undefined') return undefined;
    const targetId = resolveSectionId(initialValue || sectionId, sectionIds, sectionIds[0]);
    ...
    const timer = window.setTimeout(() => {
      ...
      if (node) {
        ...
        setPersistedSectionId(template, targetId);
      }
      restoredRef.current = true;
    }, 80);

    return () => window.clearTimeout(timer);
  }, [template, sectionIds, initialValue, sectionId]);
